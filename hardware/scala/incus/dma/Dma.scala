...
object Dma{
  ...
  case class Parameter(memConfig: PipelinedMemoryBusConfig,
                       ...
                       channels : ArrayBuffer[ChannelParameter],
                       inputs : ArrayBuffer[InputParameter] = ArrayBuffer[InputParameter](),
                       outputs : ArrayBuffer[OutputParameter] = ArrayBuffer[OutputParameter](),
  ...
  case class SDParameter(streamLengthWidth : Int,
                         burstWidth : Int,
                         memory : Boolean = true,
                         stream : Boolean = true){
  ...
  case class Dma(val p : Parameter) extends Component{
    ...
    class SourceDestinationBase(cp : ChannelParameter, val sdp : SDParameter, streamIdWidth : Int) extends Area {
      ...
      val start, dontStop = False
       ...
       //Stream capable
      val length, counter = sdp.stream generate Reg(UInt(sdp.streamLengthWidth bits))
      ...
      if(sdp.streamLengthWidth != 0 && sdp.stream) when(counterIncrement) {
        counter := counter + 1
      }
...
//          sharedRamWrite.data(0, p.inputs.map(_.dataWidth).max bits) := io.inputs.map(_.data.resized).read(stream.streamId)
          switch(stream.streamId) {
            for (i <- 0 until p.inputs.length) is(i){
              sharedRamWrite.data(0, p.inputs(i).dataWidth bits) := io.inputs(i).data
            }
          }
          streamChannels.nonEmpty generate when(stream.inputsValid.orR) {
            sharedRamWrite.valid := True
            io.inputs(stream.streamId).ready := True
    ...
    val fifoRead = new Area {
      ...
      val cmd = new Area{
        val proposal = Vec(channels.map(c =>
          c.destination.busy && !c.destination.stop && (c.destination.memory || (if(outputs.nonEmpty && c.cp.destination.stream) outputs.map(_.fillMe).read(c.destination.streamId) else False)) && !c.fifoEmpty
        ))
      ...
      val rsp = new Area{
        ...
        for((output, id) <- outputs.zipWithIndex){
          output.bufferIn.valid := input.valid && !memory && streamId === id
...
//  )
  ...
  val channels = ArrayBuffer.tabulate(4)( i =>
    Dma.ChannelParameter(
      fifoDepth = 32,
      source = Dma.SDParameter(
      ...
      destination = Dma.SDParameter(
        streamLengthWidth = if(i == 0) 12 else 4,
        burstWidth = 4,
        memory = true,
        stream = i == 0
      )
  ...
  val p = Dma.Parameter(
    ...
    channels = ArrayBuffer(
      ...
      Dma.ChannelParameter(
        fifoDepth = 16,
        source = Dma.SDParameter(
          streamLengthWidth = 14,
          burstWidth = 4,
          memory = true,
          stream = true
        ),
        destination = Dma.SDParameter(
          streamLengthWidth = 10,
          burstWidth = 4,
          memory = true,
          stream = true
        )
...
object DmaTester extends App {
  ...
  val p = Dma.Parameter(
    ...
    memoryLengthWidth = 12,
    singleMemoryPort = true,
    inputs = ArrayBuffer.fill(10)(Dma.InputParameter(dataWidth = 8*(1 << Random.nextInt(3)))),
    outputs = ArrayBuffer.fill(10)(Dma.OutputParameter(dataWidth = 8*(1 << Random.nextInt(3)))),
    channels = Dma.ChannelParameter(
      ...
      Dma.ChannelParameter(
        fifoDepth = fifoDepth,
        source = Dma.SDParameter(
          streamLengthWidth = Math.max(4, i),
...
//  SimConfig.withWave.compile(new Dma.Dma(p)).doSim("test", 42) { dut =>
  SimConfig.allOptimisation.compile(new Dma.Dma(p)).doSim("test", 42) { dut =>
    ...
    def createTask() = {
      ...
      var byteCountMax = p.memConfig.dataWidth/8
      if(!src.memory) byteCountMax = Math.min(byteCountMax, p.inputs(inputId).dataWidth/8)
      if(!dst.memory) byteCountMax = Math.min(byteCountMax, p.outputs(outputId).dataWidth/8)
      val sizeMax = log2Up(byteCountMax)
      val size = Random.nextInt(sizeMax + 1)
      val length =  Random.nextInt(1 << Math.min(if(src.memory) p.memoryLengthWidth >> size else cp.source.streamLengthWidth, if(dst.memory) p.memoryLengthWidth >> size else cp.destination.streamLengthWidth))
